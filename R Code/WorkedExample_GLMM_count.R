...
r3 <- glmmTMB(y ~ spray * lead + (1|block), data=d1, family="poisson")   ### nb w/ block
...
library(broom.mixed)
glance(r4)
tidy(r4, effects="ran_pars") ## same as stdev in summary()

r4_blups <- tidy(r4, effects="ran_vals") ## same as stdev in summary()
hist(r4_blups$estimate, breaks=seq(-1,1,l=10), main="Hist of RE blups")
...
Anova(r3) ## compare with NB to poisson
...
r2(r4)
r.squaredGLMM(r4) ## from MuMIn, lognormal is same as performance
