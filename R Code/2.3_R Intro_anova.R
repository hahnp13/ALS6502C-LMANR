...
## look reasonable but there are some problems with residuals indicating assumptions of the linear model are violated and may cause problems with coefficients and p-values
...
tidy(lm2, conf.int=TRUE) # tidy() to examine coefficients (from broom package or broom.mixed if using glmmTMB)
...
Anova(lm2, type=2)  ## car::Anova will print out an ANOVA table testing 
                     ...
                     # type = 2 provides Type II sums of squares, which is usually the best way to go. See course manual for notes about Type II vs III
...
em1 <- emmeans(lm2, ~spray) %>% as.data.frame()

ggplot() + 
  geom_boxplot(data=d, aes(x=spray,y=count), outlier.shape = NA) + 
  geom_jitter(data=d, aes(x=spray,y=count), height=0,width=.1, size=2) + 
  geom_pointrange(data=em1, aes(x=spray, y=emmean, ymin=lower.CL, ymax=upper.CL),
                color="purple", size=1.25, linewidth=2) + ##geom_errorbar() call also be used to draw error bars
  theme_bw(base_size = 14)


## re-run using glmmTMB ####
lm2a <- glmmTMB(count~spray , data=d)
Anova(lm2a) # note X2 instead of F-value. No Sums of Squares.
